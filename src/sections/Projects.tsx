...
import CheckCircleIcon from "@/assets/icons/check-circle-white.svg";
...
export const ProjectsSection = () => {
  return (
    <section className="lg:mx-[150px] pb-16 lg:py-24 mb-[10vh]">
    <div>
      <div className="container">
        ...
        <div className="flex flex-col gap-20 mt-10 md:mt-20">
          {portofolioProjects.map((project, projectIndex) => (
            <div
              key={project.title}
              className="bg-gray-900 rounded-3xl z-0 overflow-hidden after:z-10 after:content-[''] after:absolute after:inset-0 after:outline-2 after:outline after:-outline-offset-2 after:rounded-3xl after:outline-white/20 px-8 pt-8 md:pt-12 md:px-10 after:pointer-events-none lg:pt16 lg:px-20 sticky lg:h-[63vh]"style={{
            ...
            >
              ...
              <div className="lg:grid lg:grid-cols-2 lg:gap-16">
                ...
                <div className="bg-gradient-to-r from-custom-green to-custom-gray inline-flex gap-2 font-bold uppercase tracking-widest text-sm text-transparent bg-clip-text">
                  <span>{project.company}</span>
                  <span>&bull;</span>
                  <span>{project.year}</span>
                </div>
              <h3 className="font-bold text-white text-2xl mt-2 md:text-4xl md:mt-5">{project.title}</h3>
              <hr className="border-t-2 border-white/5 mt-4 md:mt-5" />
              <ul className="flex flex-col gap-4 mt-4 md:mt-5">
                {project.results.map((result) => (
                  <li key={result.title} className="flex gap-2 text-sm text-white/50 md:text-base">
                    <img src={CheckCircleIcon} className="size-5 md:size-6 opacity-30" />
              ...
              {project.link && (
                <a href={project.link}>
                  <button className="bg-white text-gray-950 h-12 w-full rounded-xl font-semibold inline-flex items-center justify-center gap-2 mt-8 md:w-auto px-6">
                    ...
                    <img src={ArrowUpIcon} className="w-4 h-4" />
                  </button>
                </a>
              )}
              </div>
